...
if (isset($_GET['fetch_rooms'])) {
    ...
    // decode the JSON string received from AJAX
    ...
    // count no. of rooms and initialize output variable
    $count_rooms = 0;
    $output = "";

    // fetching settings table for shutdown value
    $settings_q = "SELECT * FROM `settings` WHERE `sr_no`=1";
    $settings_r = mysqli_fetch_assoc(mysqli_query($con, $settings_q));

    // query for rooms cards with guest filters
    $room_res = select("SELECT * FROM `rooms` WHERE `adult` >= ? AND `children` >= ? AND `status`=? AND `removed`=?", [$adults, $children, 1, 0], 'iiii');
    while ($room_data = mysqli_fetch_assoc($room_res)) {

        // check availability of room filter
        if ($chk_avail['checkin'] != '' && $chk_avail['checkout'] != '') {
