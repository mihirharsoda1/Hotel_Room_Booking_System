...
if (isset($_POST['get_bookings'])) {
    ...
    $res = select($query, ['cancelled', 0], 'si');
    if (mysqli_num_rows($res) == 0) {
        echo "<tr><td colspan='5' class='text-center fw-bold'>No Bookings Found!</td></tr>";
        exit;
    }
    $i = 1;
    $table_data = "";
    while ($data = mysqli_fetch_assoc($res)) {
        ...
        $table_data .= "
            <tr>
                ...
                <td>
                    <button type='button' onclick='refund_bookings($data[booking_id])' class='btn btn-success btn-sm fw-bold shadow-none'>
...
if (isset($_POST['search_booking'])) {
    ...
    $query = "SELECT bo.*, bd.* FROM `booking_order` bo
        INNER JOIN `booking_details` bd ON bo.booking_id = bd.booking_id
        WHERE (bo.order_id LIKE ? 
        OR bd.user_name LIKE ? 
        OR bd.phonenum LIKE ?)
        AND bo.booking_status = ? AND bo.refund = ?
        ORDER BY bo.booking_id ASC";

    $res = select($query, ["%$search%", "%$search%", "%$search%", 'cancelled', 0], "ssssi");
